...
@router.get("")
async def list_batches(current_user: AuthUser = Depends(require_asset_batch)):
...
async def _process_batch_background(batch_id: UUID, user_id: UUID):
...
async def _send_batch_completion_notification(batch_id: UUID, user_id: UUID):
